import { arc, format, hierarchy, partition, scaleLog } from 'd3';
// color imports 
...
export const Sunburst = () => {
    ...
    //   console.log("svg", svg);
      ...
      function clicked(event, p) {
        ...
        // go through each of the elements and set d.target
        // x = theta
        ...
        root.each(
          (d) =>
            (d.target = {
              ...
              // do same for x1
              x0:   
                Math.max(0, Math.min(1, (d.x0 - p.x0) / (p.x1 - p.x0))) * 2 * Math.PI,    // Math.max(0) ensures positive num - min(1) ensures lower than 1
              x1:
      ...
      function arcVisible(d) {
        // if in between level 1 and 3 and final angle is greater than start angle - show arc
        // Note: level 1 is inner layer - then 2nd layer (first seen layer) then 3rd layer (second seen layer)
      ...
      function labelTransform(d) {
        const x = (((d.x0 + d.x1) / 2) * 180) / Math.PI;    // Convert midpoint of radial angle into degrees
        const y = ((d.y0 + d.y1) / 2) * (RADIUS);           // Multipley midpoint of levels with rad to get position
